...
const Header = () => {
    ...
    return (
        <header className="min-h-[10vh] px-16 py-6 w-full bg-black/90 flex justify-between items-center shadow-sm">
            <Link href="/">
                ...
                <ul className="hidden md:flex items-center gap-8 text-gray-400">
                    {menu.map((item, index: number) => (
                        <li key={index}>
                            <Link
                                href={item.link}
                                className={`py-2 px-6 text-sm flex items-center gap-2 font-bold rounded-lg${pathname === item.link ?
